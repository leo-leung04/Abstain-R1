...
training data for small language models on reading comprehension
with abstention and clarification.

You will be given:
- A [Question], which includes:
  - A context paragraph providing information
  - A question that requires extracting information from the context
- A label indicating that this question is UNANSWERABLE because the
  context does not contain the information needed to answer it.

Your goal is to:
1) Reason carefully about why the question CANNOT be answered from the
   provided context, and
2) Produce a user-facing response that:
   - Explicitly abstains from answering, and
   - Briefly explains what information is missing from the context.

You MUST NOT guess or invent information not present in the context.

--------------------------------------------------
DOMAIN & DATASET BEHAVIOR (SQUAD2-STYLE READING COMPREHENSION)
--------------------------------------------------
1. The SQuAD2 dataset contains reading comprehension questions that:
   - Provide a context paragraph with factual information
   - Require extracting specific information from the context to answer
   - Test the ability to locate and extract relevant information accurately
   - May involve understanding relationships, dates, names, or other specific facts

2. When judging answerability, you MUST:
   - Read the context carefully and identify all information it contains
   - Determine what information the question is asking for
   - Check if that information is present in the context
   - If the required information is not in the context, you must abstain

3. Common reasons for unanswerable questions:
   - The context does not mention the entity or fact asked about
   - The context provides related information but not the specific answer
   - The question asks about details not covered in the context
   - The context is ambiguous or unclear about the requested information

--------------------------------------------------
ABSTENTION & CLARIFICATION BEHAVIOR
--------------------------------------------------
For this task, the question is GUARANTEED to be UNANSWERABLE from the
given context. Therefore, you must:

1. In your reasoning (<thinking>):
   - Restate what the question is asking in your own words.
   - Systematically search through the context for relevant information.
   - Identify what specific information would be needed to answer the question.
   - Explain why that information is not present in the context.
   - Optionally, note what related information (if any) is present but
...
2. In your final response (<answer>):
   ...
   - After that, give a brief, user-facing clarification in natural
     language that:
       (a) Explains that the context does not contain the information
           needed to answer the question, or
       (b) States what information is missing from the context.
   - Do NOT extract or invent information not present in the context.
...
--------------------------------------------------
1. No Hindsight Bias:
   - Reason forward from the [Question] and context.
   - Do NOT act as if you magically know what the answer should be.
   - Your reasoning should show how you discovered that the context
     does not contain the answer.

2. Evidence-Centered Analysis:
   - Make it clear which parts of the context you searched.
   - Explicitly identify what information the question requires.
   - Explain why that information is not found in the context.

3. Granularity:
   - Avoid big jumps like "the context doesn't have the answer, so I abstain".
   - Instead, show the intermediate checks: what the question asks for,
     what you found in the context, what's missing, and why it's critical.

4. Self-Checking:
   - Before concluding, add a short internal check, such as:
     "I have searched the context for [specific information], but it is
     not mentioned, so the question remains unanswerable."

5. Format Compliance:
   - Do NOT repeat the full [Question] verbatim.
   - Do NOT output any answer extracted from the context.
...
Then systematically search through the context and identify what
information would be needed to answer the question.
Explain, step by step, why that information is not present in the
context.
Optionally, note what related information exists but is insufficient.
Conclude that you must abstain.
</thinking>
<answer>
\\boxed{I don't know.}
[Write a short, user-facing explanation (1–3 sentences) that either:
 - explains that the context does not contain the information needed
   to answer the question, or
 - states what specific information is missing from the context.]
</answer>
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Clean SQuAD2 unanswerable data and generate DeepSeek abstention outputs (JSONL).",
    )
    parser.add_argument(
        "--input-path",
        default=str(BASE_DIR / "squad2_unanswerable_100_forward.json"),
        help="Path to the input JSON with unanswerable entries.",
    )
    parser.add_argument(
        "--output-path",
        default=str(BASE_DIR / "squad2_unanswerable_clean_deepseek.jsonl"),
