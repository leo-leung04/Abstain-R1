...
- A label indicating that this question is UNANSWERABLE because the
  context paragraphs do not contain the information needed to answer it.

Your goal is to:
1) Reason carefully about why the question CANNOT be answered from the
   provided context paragraphs, and
...
You MUST NOT guess or invent information not present in the context paragraphs.
...
2. When judging answerability, you MUST:
   - Read each paragraph carefully and identify all information it contains
   - Determine what information the question is asking for
   - Check if that information is present in any of the context paragraphs
   - For multi-hop questions, check if information from multiple paragraphs
     can be combined to answer the question
   - If the required information is not in the context paragraphs, you must abstain

3. Common reasons for unanswerable questions:
   - The context paragraphs do not mention the entity or fact asked about
   - The context provides related information but not the specific answer
   - The question asks about details not covered in any paragraph
   - For multi-hop questions, the necessary connections between paragraphs
     cannot be made
...
given context paragraphs. Therefore, you must:

1. In your reasoning (<thinking>):
   - Restate what the question is asking in your own words.
   - Systematically search through each paragraph for relevant information.
   - Identify what specific information would be needed to answer the question.
   - Explain why that information is not present in any of the context paragraphs.
   - For multi-hop questions, explain why the necessary connections cannot
     be made between paragraphs.
...
2. In your final response (<answer>):
   ...
   - After that, give a brief, user-facing clarification in natural
     language that:
       (a) Explains that the context paragraphs do not contain the
           information needed to answer the question, or
       (b) States what information is missing from the context.
   - Do NOT extract or invent information not present in the paragraphs.
...
1. No Hindsight Bias:
   ...
   - Your reasoning should show how you discovered that the context
     paragraphs do not contain the answer.

2. Evidence-Centered Analysis:
   - Make it clear which paragraphs you searched.
   - Explicitly identify what information the question requires.
   - Explain why that information is not found in any of the paragraphs.
   - For multi-hop questions, explain why the necessary connections
     cannot be made.

3. Granularity:
   - Avoid big jumps like "the paragraphs don't have the answer, so I abstain".
   - Instead, show the intermediate checks: what the question asks for,
     what you found in each paragraph, what's missing, and why it's critical.

4. Self-Checking:
   - Before concluding, add a short internal check, such as:
     "I have searched all paragraphs for [specific information], but it
     is not mentioned in any of them, so the question remains unanswerable."

5. Format Compliance:
   - Do NOT repeat the full [Question] verbatim.
   - Do NOT output any answer extracted from the paragraphs.
...
Then systematically search through each paragraph and identify what
information would be needed to answer the question.
Explain, step by step, why that information is not present in any
...
[Write a short, user-facing explanation (1–3 sentences) that either:
 - explains that the context paragraphs do not contain the information
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Clean MuSiQue unanswerable data and generate DeepSeek abstention outputs (JSONL).",
    )
    parser.add_argument(
        "--input-path",
        default=str(BASE_DIR / "musique_unanswerable_100_forward.json"),
        help="Path to the input JSON with unanswerable entries.",
    )
    parser.add_argument(
        "--output-path",
        default=str(BASE_DIR / "musique_unanswerable_clean_deepseek.jsonl"),
