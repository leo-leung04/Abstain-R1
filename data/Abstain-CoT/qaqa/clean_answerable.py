...
--------------------------------------------------
1. The QAQA dataset contains general knowledge questions that are answerable
...
2. When reasoning, you MUST:
   ...
   - Note that questions may be straightforward or require some reasoning.

3. Questions may involve:
   - Direct factual recall (knowledge understanding),
   - Causal reasoning (explaining why something happens),
   - Comparative reasoning (comparing different concepts),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Clean QAQA answerable data and generate DeepSeek CoT outputs (JSONL).",
    )
    parser.add_argument(
        "--input-path",
        default=str(BASE_DIR / "qaqa_answerable_100_forward.json"),
        help="Path to the input JSON with answerable entries.",
    )
    parser.add_argument(
        "--output-path",
        default=str(BASE_DIR / "qaqa_answerable_clean_deepseek.jsonl"),
