...
--------------------------------------------------
1. The Known-Unknown Questions dataset contains general knowledge questions
   that may be unanswerable due to:
...
3. Common issues in unanswerable questions:
   ...
   - Questions with multiple-choice options where none are clearly correct
     or the question itself is flawed
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Clean Known-Unknown Questions unanswerable data and generate DeepSeek abstention outputs (JSONL).",
    )
    parser.add_argument(
        "--input-path",
        default=str(BASE_DIR / "known_unknown_questions_unanswerable_100_forward.json"),
        help="Path to the input JSON with unanswerable entries.",
    )
    parser.add_argument(
        "--output-path",
        default=str(BASE_DIR / "known_unknown_questions_unanswerable_clean_deepseek.jsonl"),
