...
- A [Question], which is a natural language question covering various topics
  including history, culture, science, geography, and general knowledge.
- A [Ground Truth Answer], which is the correct answer for this question.

Your objective is to generate a clear, step-by-step Chain of Thought (CoT)
that logically leads from the question to the correct answer.

You MUST behave as if you are solving the problem from scratch and MUST
NOT reveal or hint that you were given the answer.

--------------------------------------------------
DOMAIN & DATASET BEHAVIOR (KNOWN-UNKNOWN-QUESTIONS-STYLE GENERAL QA)
--------------------------------------------------
1. The Known-Unknown Questions dataset contains general knowledge questions that:
   - Cover a wide range of topics (history, culture, science, geography, etc.)
   - May be factual recall questions or require reasoning
   - Test knowledge that is generally known or can be inferred
   - May involve multiple-choice style questions or open-ended questions

2. When reasoning, you MUST:
   - Use your general knowledge to provide accurate, factual answers.
   - For factual questions, recall or derive the relevant information.
   - For questions with multiple options, evaluate each option systematically.
   - For explanatory questions, provide clear, step-by-step explanations.
   - Consider the context and domain of the question when reasoning.

3. Questions may involve:
   - Direct factual recall (knowledge understanding),
   - Comparative reasoning (comparing options or concepts),
   - Causal reasoning (explaining why something happens),
   - Multi-step reasoning that combines multiple facts.
...
2. Evidence-Centered Reasoning:
   ...
   - Identify the key concepts, entities, or facts involved.
   - For questions with options, evaluate each option systematically.
   - For explanatory questions, break down the explanation into logical steps.
   - Clearly connect your reasoning to the final answer.

3. Granularity:
   - Avoid big logical jumps. Break the reasoning into small, checkable steps.
   - For factual questions, explain how you recall or derive the information.
   - For comparative questions, make it clear why one option is better than others.
   - For explanatory questions, make each step of the explanation explicit.

4. Self-Checking:
   - Before concluding, add a short internal check such as
     "This answer is correct because...".
   - Verify that the answer directly addresses the question asked.
   - For multiple-choice questions, verify that other options are less appropriate.
...
Then, step by step, reason through the question using relevant knowledge
or logical steps to arrive at the answer.
Finish by briefly checking that the answer is correct and complete.
</thinking>
<answer>
{ground_truth}
</answer>
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Clean Known-Unknown Questions answerable data and generate DeepSeek CoT outputs (JSONL).",
    )
    parser.add_argument(
        "--input-path",
        default=str(BASE_DIR / "known_unknown_questions_answerable_100_forward.json"),
        help="Path to the input JSON with answerable entries.",
    )
    parser.add_argument(
        "--output-path",
        default=str(BASE_DIR / "known_unknown_questions_answerable_clean_deepseek.jsonl"),
