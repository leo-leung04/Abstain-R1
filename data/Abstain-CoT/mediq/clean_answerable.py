...
- A [Ground Truth Answer], which is the correct answer to the medical question.

Your objective is to generate a clear, step-by-step Chain of Thought (CoT)
that logically leads from the medical context to the correct answer.
...
2. When reasoning, you MUST:
   - Carefully read the clinical context and identify key findings (symptoms, signs, lab results, etc.)
   - Apply medical knowledge to interpret the clinical presentation
   - For diagnostic questions, consider differential diagnoses and identify the most likely diagnosis
   - For treatment questions, consider appropriate therapeutic options based on the condition
   - Evaluate each option systematically if multiple choices are provided
   - Use clinical reasoning to connect findings to the correct answer

3. Clinical reasoning approach:
   - Identify the chief complaint and key clinical findings
   - Consider relevant pathophysiology and disease mechanisms
   - Evaluate differential diagnoses or treatment options
   - Select the most appropriate answer based on clinical evidence
   - Consider contraindications, side effects, and clinical guidelines when relevant
...
1. No Hindsight Bias:
   ...
   - The CoT should look like a natural clinical reasoning process.

2. Evidence-Centered Reasoning:
   - Start by summarizing the clinical scenario in your own words.
   - Identify key clinical findings (symptoms, signs, lab values, imaging, etc.)
   - Apply medical knowledge to interpret these findings.
   - For diagnostic questions, explain the reasoning for the diagnosis.
   - For treatment questions, explain why a particular treatment is appropriate.
   - If multiple options are provided, evaluate each systematically.
   - Clearly connect clinical evidence to the final answer.

3. Granularity:
   - Avoid big logical jumps. Break the reasoning into small, checkable steps.
   - Explain how specific clinical findings support the diagnosis or treatment choice.
   - For differential diagnosis, explain why other options are less likely.
   - Make the clinical reasoning process explicit and logical.

4. Self-Checking:
   - Before concluding, verify that:
     - All key clinical findings have been considered
     - The reasoning is consistent with medical knowledge
     - The answer directly addresses the question asked
   - Add a brief check such as "This diagnosis/treatment is most appropriate because...".
...
Output Structure:
<thinking>
Begin by summarizing the clinical scenario and what the question is asking.
Then, step by step, identify key clinical findings, apply medical knowledge,
and reason through to arrive at the answer.
For multiple-choice questions, evaluate each option systematically.
Finish by verifying that the answer is correct and well-supported by the clinical evidence.
</thinking>
<answer>
{ground_truth}
</answer>
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Clean MediQ answerable data and generate DeepSeek CoT outputs (JSONL).",
    )
    parser.add_argument(
        "--input-path",
        default=str(BASE_DIR / "mediq_answerable_100_forward.json"),
        help="Path to the input JSON with answerable entries.",
    )
    parser.add_argument(
        "--output-path",
        default=str(BASE_DIR / "mediq_answerable_clean_deepseek.jsonl"),
