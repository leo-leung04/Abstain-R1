...
--------------------------------------------------
1. The FalseQA dataset contains general knowledge questions that may be
   unanswerable due to:
...
3. Common issues in unanswerable questions:
   - Questions that assume incorrect facts (e.g., "Why carbon dioxide is
     composed of oxygen?" - CO2 contains carbon and oxygen, not just oxygen)
   - Questions with biological impossibilities (e.g., asking about male
     pregnancy)
   - Questions with logical contradictions (e.g., asking if a hegemon ranks
     second when hegemon means first)
   - Questions with taxonomic errors (e.g., asking about a species in the
     wrong family)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Clean FalseQA unanswerable data and generate DeepSeek abstention outputs (JSONL).",
    )
    parser.add_argument(
        "--input-path",
        default=str(BASE_DIR / "falseqa_unanswerable_100_forward.json"),
        help="Path to the input JSON with unanswerable entries.",
    )
    parser.add_argument(
        "--output-path",
        default=str(BASE_DIR / "falseqa_unanswerable_clean_deepseek.jsonl"),
